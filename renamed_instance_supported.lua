getgenv().Hxdwp enchantedACDebug = false

local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

local function acLog(reason)
    if getgenv().Hxdwp enchantedACDebug then
...
local function Hxdwp enchantedShowLoadingNotification()
    local RunService = game:GetService("RunService")
    local guiParent = (gethui and gethui()) or game:GetService("CoreGui")
    local gui = Hxdwp enchanted.new("ScreenGui")
    gui.Name = "Hxdwp enchantedLoadingNotification"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.DisplayOrder = 696769676967
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    gui.Parent = guiParent
    local label = Hxdwp enchanted.new("TextLabel")
...
getgenv().Hxdwp enchantedModuleCache = getgenv().Hxdwp enchantedModuleCache or {}

local function Hxdwp enchantedSafeRequire(moduleRef, timeoutSec)
    local cache = getgenv().Hxdwp enchantedModuleCache
    local key = typeof(moduleRef) == "Hxdwp enchanted" and moduleRef:GetFullName() or tostring(moduleRef)
    if cache[key] ~= nil then return cache[key] end
    local deadline = timeoutSec and (os.clock() + timeoutSec) or math.huge
    local lastErr
    while os.clock() < deadline do
        if typeof(moduleRef) == "Hxdwp enchanted" and not moduleRef.Parent then
            task.wait(0.1)
        else
    ...
    error("[Hxdwp enchanted] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end

getgenv().Hxdwp enchantedRequire = Hxdwp enchantedSafeRequire

local function Hxdwp enchantedPreloadCoreModules()
    ...
    task.spawn(function() pcall(function() Hxdwp enchantedSafeRequire(modulesFolder:WaitForChild("EnumLibrary", 4)) end) end)
    task.spawn(function() pcall(function() Hxdwp enchantedSafeRequire(modulesFolder:WaitForChild("Utility", 4)) end) end)
    local controllers = playerScripts:WaitForChild("Controllers", 8)
    task.spawn(function() pcall(function() Hxdwp enchantedSafeRequire(controllers:WaitForChild("FighterController", 4)) end) end)
    task.spawn(function() pcall(function() Hxdwp enchantedSafeRequire(controllers:WaitForChild("CameraController", 4)) end) end)
end

local function Hxdwp enchantedHideErrorPromptsStep()
...
local function Hxdwp enchantedRunLoadingBootstrap()
    local runService = game:GetService("RunService")
    local dismissLoading = Hxdwp enchantedShowLoadingNotification()
    local hideErrors = true
    local hideConn = runService.RenderStepped:Connect(function()
        if hideErrors then Hxdwp enchantedHideErrorPromptsStep() end
    end)

    pcall(Hxdwp enchantedPreloadCoreModules)
...
local RunService = game:GetService("RunService")
Hxdwp enchantedRunLoadingBootstrap()
...
local function Hxdwp enchantedPlainReplace(str, find, replace)
    local startPos = 1
    while true do
...
getgenv().Hxdwp enchantedSliderFillState = getgenv().Hxdwp enchantedSliderFillState or {}
getgenv().Hxdwp enchantedSliderFollowRate = getgenv().Hxdwp enchantedSliderFollowRate or 11

getgenv().Hxdwp enchantedSetSliderFill = function(fill, targetX, hideBorder, sliderObj, forceSnap)
    if not fill or not fill.Parent then return end
    targetX = math.floor(tonumber(targetX) or 0)
    local stateMap = getgenv().Hxdwp enchantedSliderFillState
    if forceSnap then
...
if not getgenv().Hxdwp enchantedSliderFillBound then
    getgenv().Hxdwp enchantedSliderFillBound = true
    RunService.RenderStepped:Connect(function(dt)
        local stateMap = getgenv().Hxdwp enchantedSliderFillState
        if not stateMap then return end
        dt = math.clamp(tonumber(dt) or 0, 1 / 1000, 1 / 15)
        local follow = getgenv().Hxdwp enchantedSliderFollowRate or 11
...
local libStr, themeStr, saveStr
local httpDone = 0
task.spawn(function() libStr = game:HttpGet(repo .. "Library.lua"); httpDone += 1 end)
task.spawn(function() themeStr = game:HttpGet(repo .. "addons/ThemeManager.lua"); httpDone += 1 end)
task.spawn(function() saveStr = game:HttpGet(repo .. "addons/SaveManager.lua"); httpDone += 1 end)
repeat task.wait() until httpDone >= 3

local Library = loadstring(libStr)()
local ThemeManager = loadstring(themeStr)()
local SaveManager = loadstring(saveStr)()
...
do
    ...
    function Library:MakeDraggable(Hxdwp enchanted, Cutoff)
        Hxdwp enchanted.Active = true
        Hxdwp enchanted.InputBegan:Connect(function(Input)
            if Input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
            local mPos = UIS:GetMouseLocation()
            local dX = mPos.X - Hxdwp enchanted.AbsolutePosition.X
            local dY = mPos.Y - Hxdwp enchanted.AbsolutePosition.Y
            if dY > (Cutoff or 40) then return end
            while UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                local mp = UIS:GetMouseLocation()
                Hxdwp enchanted.Position = UDim2.fromOffset(mp.X - dX, mp.Y - dY)
...
local function applyHxdwp enchantedAccentTheme()
...
applyHxdwp enchantedAccentTheme()
getgenv().Hxdwp enchantedApplyAccentTheme = applyHxdwp enchantedAccentTheme
...
local function applyHxdwp enchantedUiLayering()
    pcall(function()
    ...
    pcall(function()
        local st = getgenv().Hxdwp enchantedCosmeticUIState
    ...
    pcall(function()
        local kb = getgenv().Hxdwp enchantedKeybindList
...
getgenv().Hxdwp enchantedApplyUiLayering = applyHxdwp enchantedUiLayering

local Window = Library:CreateWindow({
    Title = 'Hxdwp enchanted | Rivals',
    Center = true,
    AutoShow = true,
    TabPadding = 6,
    MenuFadeTime = 0,
...
applyHxdwp enchantedUiLayering()

getgenv().Hxdwp enchantedMenuCursor = getgenv().Hxdwp enchantedMenuCursor or {}

local function ensureHxdwp enchantedMenuCursor()
    local mc = getgenv().Hxdwp enchantedMenuCursor
...
local function isHxdwp enchantedMenuOpen()
...
local function shouldSuppressGameplayOverlays()
    return isHxdwp enchantedMenuOpen()
end

ensureHxdwp enchantedMenuCursor()

local MENU_CURSOR_HZ = 240
local MENU_CURSOR_BIND = "Hxdwp enchantedMenuCursorDraw"
local menuCursorAccum = 0

local function drawHxdwp enchantedMenuCursor()
    local mc = getgenv().Hxdwp enchantedMenuCursor
    if not mc or not mc.cursor or not mc.outline then return end
    local open = isHxdwp enchantedMenuOpen()
    if not open then
...
pcall(function()
    RunService:BindToRenderStep(MENU_CURSOR_BIND, Enum.RenderPriority.Last.Value + 10, function(dt)
        ...
        drawHxdwp enchantedMenuCursor()
    end)
end)

getgenv().Hxdwp enchantedIsMenuOpen = isHxdwp enchantedMenuOpen

getgenv().Hxdwp enchantedLanguage = getgenv().Hxdwp enchantedLanguage or "english"

local Hxdwp enchantedWeaponTranslations = {
    korean = {
...
local Hxdwp enchantedLocaleTable = {
    english = {
...
getgenv().Hxdwp enchantedTranslateWeapon = function(name)
    if not name or type(name) ~= "string" then return name end
    local lang = getgenv().Hxdwp enchantedLanguage or "english"
    if lang == "english" then return name end
    local pack = Hxdwp enchantedWeaponTranslations[lang]
    return (pack and pack[name]) or name
end

getgenv().Hxdwp enchantedTranslateLabel = function(text)
    if not text or type(text) ~= "string" then return text end
    local lang = getgenv().Hxdwp enchantedLanguage or "english"
    if lang == "english" then return text:lower() end
    local translated = getgenv().Hxdwp enchantedTranslateWeapon(text)
    if translated ~= text then return translated:lower() end
    return text:lower()
end

getgenv().Hxdwp enchantedGetUiFont = function()
    if getgenv().Hxdwp enchantedLanguage == "korean" and getgenv().Hxdwp enchantedUIFont then
        return getgenv().Hxdwp enchantedUIFont
    end
    if Library and Library.Font then
        return Library.Font
    end
    return getgenv().Hxdwp enchantedUIFont or Enum.Font.Roboto
end

getgenv().Hxdwp enchantedL = function(key, ...)
    local lang = getgenv().Hxdwp enchantedLanguage or "english"
    local pack = Hxdwp enchantedLocaleTable[lang] or Hxdwp enchantedLocaleTable.english
    local template = pack[key] or Hxdwp enchantedLocaleTable.english[key] or key
    if select("#", ...) > 0 then
...
local Hxdwp enchantedFontHttp = game:GetService("HttpService")

local Hxdwp enchantedFontSources = {
    english = { file = "Hxdwp enchanted_ui_en.otf", fontFile = "Hxdwp enchanted_ui_en.font", name = "Hxdwp enchantedUI_En", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
    korean = { file = "Hxdwp enchanted_ui_ko.otf", fontFile = "Hxdwp enchanted_ui_ko.font", name = "Hxdwp enchantedUI_Ko", url = "https://github.com/googlefonts/noto-cjk/raw/main/Sans/OTF/Korean/NotoSansCJKkr-Regular.otf" },
    spanish = { file = "Hxdwp enchanted_ui_es.otf", fontFile = "Hxdwp enchanted_ui_es.font", name = "Hxdwp enchantedUI_Es", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
}

local function loadHxdwp enchantedLanguageFont(lang)
    lang = lang or getgenv().Hxdwp enchantedLanguage or "english"
    local src = Hxdwp enchantedFontSources[lang] or Hxdwp enchantedFontSources.english
    local ok, face = pcall(function()
        if isfile and writefile and getcustomasset then
            ...
            writefile(src.fontFile, Hxdwp enchantedFontHttp:JSONEncode(fontdata))
            return Font.new(getcustomasset(src.fontFile))
        end
        return nil
    end)
    if ok and face then
        getgenv().Hxdwp enchantedUIFont = face
        return face
    end
    getgenv().Hxdwp enchantedUIFont = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium)
    return getgenv().Hxdwp enchantedUIFont
end

getgenv().Hxdwp enchantedReloadLanguageFont = function(lang)
    lang = lang or getgenv().Hxdwp enchantedLanguage or "english"
    if lang == "english" and Library and Library.Font then
        getgenv().Hxdwp enchantedUIFont = Library.Font
        return Library.Font
    end
    return loadHxdwp enchantedLanguageFont(lang)
end
task.defer(function()
    pcall(function()
        getgenv().Hxdwp enchantedReloadLanguageFont(getgenv().Hxdwp enchantedLanguage)
    end)
end)

getgenv().Hxdwp enchantedDrawingTextPool = getgenv().Hxdwp enchantedDrawingTextPool or {}

getgenv().Hxdwp enchantedTrackDrawingText = function(obj)
    table.insert(getgenv().Hxdwp enchantedDrawingTextPool, obj)
    pcall(function()
        local face = getgenv().Hxdwp enchantedUIFont
        if face and obj.FontFace ~= nil then obj.FontFace = face end
    end)
    return obj
end

getgenv().Hxdwp enchantedApplyDrawingUIFont = function()
    local face = getgenv().Hxdwp enchantedUIFont
    if not face then return end
    for _, obj in ipairs(getgenv().Hxdwp enchantedDrawingTextPool) do
        pcall(function()
...
getgenv().Hxdwp enchantedApplyUiFont = function(obj, textSize)
    if not obj then return end
    if textSize then obj.TextSize = textSize end
    local font = getgenv().Hxdwp enchantedGetUiFont()
    pcall(function()
...
getgenv().Hxdwp enchantedApplyUIFont = function()
    local face = getgenv().Hxdwp enchantedGetUiFont()
    if not face then return end
    getgenv().Hxdwp enchantedUIFont = face
        ...
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
                    if typeof(lbl) == "Hxdwp enchanted" and lbl:IsA("TextLabel") then
                        lbl.FontFace = face
                    end
                end
            end
    ...
    if getgenv().Hxdwp enchantedApplyDrawingUIFont then pcall(getgenv().Hxdwp enchantedApplyDrawingUIFont) end
    if getgenv().Hxdwp enchantedApplyKbListTheme then pcall(getgenv().Hxdwp enchantedApplyKbListTheme) end
end

getgenv().Hxdwp enchantedHudFonts = {
    ...
    { n = "custom", draw = 2, face = function() return getgenv().Hxdwp enchantedUIFont or Font.fromEnum(Enum.Font.Gotham) end },
}
getgenv().Hxdwp enchantedHudFontNames = {}
for i, slot in ipairs(getgenv().Hxdwp enchantedHudFonts) do
    getgenv().Hxdwp enchantedHudFontNames[i] = slot.n
end
getgenv().Hxdwp enchantedHudFontIdx = 1
getgenv().Hxdwp enchantedHudDrawFont = 2

getgenv().Hxdwp enchantedHudFontIndexByName = function(name)
    for i, slot in ipairs(getgenv().Hxdwp enchantedHudFonts) do
...
getgenv().Hxdwp enchantedApplyHudFont = function(idx)
    idx = math.clamp(tonumber(idx) or getgenv().Hxdwp enchantedHudFontIdx or 1, 1, #getgenv().Hxdwp enchantedHudFonts)
    local slot = getgenv().Hxdwp enchantedHudFonts[idx]
    getgenv().Hxdwp enchantedHudFontIdx = idx
    getgenv().Hxdwp enchantedHudDrawFont = slot.draw or 2
    local face
    pcall(function() face = slot.face() end)
    if face then getgenv().Hxdwp enchantedEspUiFont = face end
    if _G.ESPObjects then
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
                    if typeof(lbl) == "Hxdwp enchanted" and lbl:IsA("TextLabel") and face then
                        lbl.FontFace = face
                    end
                end
            end
    ...
    for _, obj in ipairs(getgenv().Hxdwp enchantedDrawingTextPool or {}) do
        pcall(function()
            if obj.Font ~= nil then obj.Font = getgenv().Hxdwp enchantedHudDrawFont end
            if face and obj.FontFace ~= nil then obj.FontFace = face end
        end)
    end
end
...
local function alignHxdwp enchantedMenuTabs()
...
task.defer(alignHxdwp enchantedMenuTabs)
...
local Utility           = Hxdwp enchantedSafeRequire(ReplicatedStorage.Modules.Utility)
local EnumLibrary       = Hxdwp enchantedSafeRequire(ReplicatedStorage.Modules.EnumLibrary)
local LocalPlayer       = Players.LocalPlayer
...
getgenv().Hxdwp enchantedGetAspectStretch = getAspectStretch
getgenv().Hxdwp enchantedWorldToScreen = worldToScreen
getgenv().Hxdwp enchantedScreenAnchor = screenAnchor
getgenv().Hxdwp enchantedScreenCenter = screenCenter
getgenv().Hxdwp enchantedGetUnstretchedCameraCFrame = getUnstretchedCameraCFrame

_G.Features = _G.Features or {}
_G.Features.CrossbowSoundId = "rbxassetid://165946246"
getgenv().Hxdwp enchantedCombatLastShotAt = getgenv().Hxdwp enchantedCombatLastShotAt or 0

local function markCombatShot()
    getgenv().Hxdwp enchantedCombatLastShotAt = tick()
end
getgenv().Hxdwp enchantedMarkCombatShot = markCombatShot

getgenv().Hxdwp enchantedWorldToScreenEsp = function(worldPos, cam)
...
local fovScreenGui = Hxdwp enchanted.new("ScreenGui")
...
local function buildfov(name, cfg)
    local container = Hxdwp enchanted.new("Frame")
    container.Name                   = name
    ...
    local fill = Hxdwp enchanted.new("Frame")
    ...
    local fillCorner = Hxdwp enchanted.new("UICorner")
    fillCorner.CornerRadius = UDim.new(1, 0)
    fillCorner.Parent       = fill

    local fillgrad = Hxdwp enchanted.new("UIGradient")
    ...
    local outline = Hxdwp enchanted.new("Frame")
    ...
    local outlineCorner = Hxdwp enchanted.new("UICorner")
    outlineCorner.CornerRadius = UDim.new(1, 0)
    outlineCorner.Parent       = outline

    local stroke = Hxdwp enchanted.new("UIStroke")
    ...
    local strokegrad = Hxdwp enchanted.new("UIGradient")
...
local function makebullettracer(pos3, endPos, isHit)
    ...
    local a0   = Hxdwp enchanted.new("Attachment"); a0.Parent = workspace.Terrain
    local a1   = Hxdwp enchanted.new("Attachment"); a1.Parent = workspace.Terrain
    local beam = Hxdwp enchanted.new("Beam")
    beam.Attachment0 = a0
    beam.Attachment1 = a1
    ...
    if cfg.glow > 0.3 then
        glow            = Hxdwp enchanted.new("PointLight")
...
local function restoreGunSoundVolumes()
    local function tryRestore(root)
        ...
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("Sound") and isRivalsGunSound(d) and not d:GetAttribute("Hxdwp enchantedShootSound") then
                pcall(function()
                    if d.Volume <= 0 then
                        d.Volume = 1
                    end
...
local function isRivalsGunSound(sound)
    if typeof(sound) ~= "Hxdwp enchanted" or not sound:IsA("Sound") then
        return false
    end
    if sound:GetAttribute("Hxdwp enchantedShootSound") then
        return false
    end
...
local function bindGunSoundMuteGuard(sound)
    if typeof(sound) ~= "Hxdwp enchanted" or not sound:IsA("Sound") or sound:GetAttribute("Hxdwp enchantedShootSound") then
        return
    end
    if not sound:GetAttribute("Hxdwp enchantedGunMuteBound") then
        sound:SetAttribute("Hxdwp enchantedGunMuteBound", true)
...
getgenv().Hxdwp enchantedRefreshGunSoundMute = refreshGunSoundMute
...
local function handleLocalShot(bypassCooldown)
    ...
    local endPos
    local hitHxdwp enchanted

    if muzzlePos then
        local camCFrame = Camera.CFrame
        endPos = camCFrame.Position + camCFrame.LookVector * 1000
        local params = RaycastParams.new()
        params.FilterDescendantsHxdwp enchanteds = { LocalPlayer.Character }
        params.FilterType = Enum.RaycastFilterType.Blacklist
        local res = workspace:Raycast(muzzlePos, (endPos - muzzlePos).Unit * 1000, params)
        if res then
            endPos = res.Position
            hitHxdwp enchanted = res.Hxdwp enchanted
        end
    end

    if hitHxdwp enchanted then
        notifyProjectileImpact(hitHxdwp enchanted)
    end
    ...
    makebullettracer(muzzlePos, endPos, hitHxdwp enchanted ~= nil)
end

getgenv().Hxdwp enchantedHandleLocalShot = handleLocalShot
...
local function playHS()
    ...
    local snd    = Hxdwp enchanted.new("Sound")
    snd.SoundId  = id
    snd.Volume   = hitSound.volume
    snd.Pitch    = hitSound.pitch
    local cam    = workspace.CurrentCamera
    if cam then
        local att = Hxdwp enchanted.new("Attachment")
        att.Parent = cam
        snd.Parent = att
    else
        ...
        if myChar and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            if tr then
                ...
                if dist <= 1500 then
                    ...
                    local params = RaycastParams.new()
                    params.FilterDescendantsHxdwp enchanteds = {myChar, char}
                    params.FilterType = Enum.RaycastFilterType.Exclude
                    local res     = workspace:Raycast(myChar.HumanoidRootPart.Position, dir*dist, params)
                    local visible = true
                    if res then
                        local hp = res.Hxdwp enchanted.Parent
...
local AIMBOT_RENDER_BIND = "Hxdwp enchantedAimbotUpdate"
...
local fonts = {}
local Hxdwp enchantedUIFont = getgenv().Hxdwp enchantedUIFont
fonts.main = (typeof(Hxdwp enchantedUIFont) == "Font" and Hxdwp enchantedUIFont) or (Hxdwp enchantedUIFont and Font.fromEnum(Hxdwp enchantedUIFont)) or Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Black)
...
local FORTNIGHT_FONT_URL = "https://raw.githubusercontent.com/ziannebanoragy/Hxdwp enchantedALPHA/refs/heads/main/fortnight.otf"
...
local function spawnHitRing(targetPart, color, config)
    config = config or {}
    local ring = Hxdwp enchanted.new("Part")
...
local function spawnHitFlash(targetPart, color, brightness, range, duration)
    ...
    local light = Hxdwp enchanted.new("PointLight")
    light.Name = "HitFlash"
    light.Color = color
...
local function spawnHitEmitter(targetPart, config)
    ...
    local emitter = Hxdwp enchanted.new("ParticleEmitter")
...
local function getActiveParticles()
    local count = 0
    local function countInHxdwp enchanted(inst)
        for _, child in ipairs(inst:GetChildren()) do
            if child:IsA("ParticleEmitter") then
                count = count + 1
            end
            countInHxdwp enchanted(child)
        end
    end
    countInHxdwp enchanted(workspace.CurrentCamera)
    return count
end
...
local function fortniteffect(adornee, color, damageText)
    ...
    local bb = Hxdwp enchanted.new("BillboardGui")
    ...
    local lbl = Hxdwp enchanted.new("TextLabel", bb)
...
local function playHS()
    ...
    local sound = Hxdwp enchanted.new("Sound")
...
hitSoundsTab:AddToggle("DisableGunSounds", {
    ...
    Callback = function(val)
        ...
        if val then
            if getgenv().Hxdwp enchantedRefreshGunSoundMute then
                getgenv().Hxdwp enchantedRefreshGunSoundMute()
            end
        else
...
local chamOutlineStore = Hxdwp enchanted.new("Folder")
...
local function buildChamOutlineGlow(anchorPart, sourcePart, cloneTable, name, scale, transparency, padding)
    ...
    if not glow or not glow.Parent then
        ...
        local weld = Hxdwp enchanted.new("WeldConstraint")
...
local function syncChamOutline(part, color)
    ...
    if not highlight or not highlight.Parent then
        highlight = Hxdwp enchanted.new("Highlight")
...
local function clearVmPartHighlight(part)
    ...
    local legacy = part and part:FindFirstChild("Hxdwp enchantedVmPartHighlight")
...
local function syncVmPartHighlight(part, fillColor, outlineColor, fillTrans, outlineTrans)
    ...
    clearVmPartHighlight(part)
    local hl = Hxdwp enchanted.new("Highlight")
    hl.Name = "Hxdwp enchantedVmPartHighlight"
...
local applyChams
local chamLoop
local CHAM_RENDER_BIND = "Hxdwp enchantedViewModelChams"
...
getgenv().Hxdwp enchantedUpdViewModelChams = updViewModelEffectLoop

applyChams = function(dt)
    if isHxdwp enchantedMenuOpen and isHxdwp enchantedMenuOpen() then
        return
    end
...
local function viewportloop()
    ...
    viewportLoop = game:GetService("RunService").RenderStepped:Connect(function()
        if isHxdwp enchantedMenuOpen and isHxdwp enchantedMenuOpen() then return end
...
local function bindViewModelAnimator(animator)
    if typeof(animator) ~= "Hxdwp enchanted" or not animator:IsA("Animator") then
        return
    end
    if animator:GetAttribute("Hxdwp enchantedNoAnimBound") then
        return
    end
    animator:SetAttribute("Hxdwp enchantedNoAnimBound", true)
...
local function hookViewModelAnimations()
    ...
    if fp then
        ...
        if not vm:GetAttribute("Hxdwp enchantedNoAnimVmHooked") then
            vm:SetAttribute("Hxdwp enchantedNoAnimVmHooked", true)
...
local function stopNoAnimLoop()
    ...
    local vm = Workspace:FindFirstChild("ViewModels")
    if vm then
        vm:SetAttribute("Hxdwp enchantedNoAnimVmHooked", nil)
        for _, desc in ipairs(vm:GetDescendants()) do
            if desc:IsA("Animator") then
                desc:SetAttribute("Hxdwp enchantedNoAnimBound", nil)
            end
        end
    end
end
...
local function makeBeamTracer(pos3, endPos)
    local a0 = Hxdwp enchanted.new("Attachment")
    a0.Parent = workspace.Terrain
    local a1 = Hxdwp enchanted.new("Attachment")
...
local modules = {
    enums = Hxdwp enchantedSafeRequire(replicatedstorage.Modules.EnumLibrary),
    fighter = Hxdwp enchantedSafeRequire(player.PlayerScripts.Controllers.FighterController),
    camcontrol = Hxdwp enchantedSafeRequire(player.PlayerScripts.Controllers.CameraController),
    utility = Hxdwp enchantedSafeRequire(replicatedstorage.Modules.Utility)
}

local MATCH_ID_ATTRS = {"MatchId", "DuelId", "RoundId", "GameId", "MatchUUID", "ArenaId", "Hxdwp enchantedId"}
...
local _Hxdwp enchantedLobbyCache = { t = 0, v = true }
local function lobbyCache()
    local now = os.clock()
    if now - _Hxdwp enchantedLobbyCache.t < 0.15 then
        return _Hxdwp enchantedLobbyCache.v
    end
    _Hxdwp enchantedLobbyCache.t = now
    _Hxdwp enchantedLobbyCache.v = not liveMatch()
    return _Hxdwp enchantedLobbyCache.v
end

getgenv().Hxdwp enchantedIsInActiveMatch = liveMatch
getgenv().Hxdwp enchantedIsInLobby = lobbyCache
...
local function getammo()
    ...
    local success, controller = pcall(function()
        return Hxdwp enchantedSafeRequire(player.PlayerScripts.Controllers.FighterController)
    end)
    ...
    for _, targetplayer in players:GetPlayers() do
        if targetplayer ~= player and targetplayer.Character then
            ...
            if valid(char) then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local wts = getgenv().Hxdwp enchantedWorldToScreen or worldToScreen
...
local function stopsync()
    ...
    clrVoidSnap()
    config.voidspam.phase = nil
    if config.orbit.savedpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart and not getgenv().Hxdwp enchantedUndergroundEnabled then
...
local function startsync()
    ...
    config.orbit.connection = runsvc.Heartbeat:Connect(function(dt)
        ...
        local isKnife = getweapon() == "Knife"
        if config.target.rightClickKnife and isKnife then
            local dist = config.orbit.radius > 0 and config.orbit.radius or 2.5
            local behindOffset = -root.CFrame.LookVector * dist + Vector3.new(0, config.orbit.height, 0)
            config.orbit.serverpos = targetpos + behindOffset
        else
        ...
        if not getgenv().Hxdwp enchantedUndergroundEnabled then
            if localfighter and localfighter.Entity and localfighter.Entity.RootPart then
                local currentPos = localfighter.Entity.RootPart.CFrame
                local newPos
                if config.target.rightClickKnife and isKnife then
                    newPos = CFrame.lookAt(config.orbit.serverpos, targetpos)
                else
...
modules.camcontrol.Update = function(...)
    if getgenv().Hxdwp enchantedUndergroundEnabled then
        return oldcamupdate(...)
...
local function updatesling()
    ...
    if slingon then
        ...
        if getgenv().Hxdwp enchantedSetUnderground then
            getgenv().Hxdwp enchantedSetUnderground(true)
        end
        ...
        if Toggles.AntiAimUnderground and not Toggles.AntiAimUnderground.Value then
            if getgenv().Hxdwp enchantedSetUnderground then
                getgenv().Hxdwp enchantedSetUnderground(false)
            end
        end
        vhState.active = false
    end
...
runsvc.RenderStepped:Connect(function()
    local inLobby = getgenv().Hxdwp enchantedIsInLobby and getgenv().Hxdwp enchantedIsInLobby()
...
local function mkStatusLbl()
    local label = getgenv().Hxdwp enchantedTrackDrawingText(Drawing.new("Text"))
    label.Size = 13
    label.Font = 2
    label.Outline = true
    label.Center = true
...
local function rageLines()
    ...
    if config.state.reloading then
        return "ragebot: voidhide", "reloading"
    end
    if inVoid() then
        local det = vsDetail()
        return "ragebot: voidhide", det ~= "" and det or "void hide"
    end
    if isKilling() then
        local name = killNm()
    ...
    if rageActive() and config.target.enabled and config.target.character then
        local det = vsDetail()
        return "ragebot: voidhide", det ~= "" and det or "waiting for target"
    end
    return "ragebot: idle", ""
end
...
rageui.ragebottab:AddToggle("RightClickKnife", {
...
local function startBackstab()
            ...
            if type(v) == "table" then
                if rawget(v, "StartShooting") and rawget(v, "IsKnife") then
                    ...
                    v.StartShooting = function(self2, ...)
                        ...
                        if config.backstab.enabled and results[3] and type(results[3]) == "table" then
    ...
    task.spawn(function()
        while config.backstab.enabled do
            pcall(function()
                local char = backstabPlayer.Character
                if not char then return end
                ...
                for _, p in ipairs(backstabPlayers:GetPlayers()) do
                    if p == backstabPlayer then continue end
                ...
                if closest then
                    ...
                    hrp.CFrame = CFrame.new(backPos, closest.Position) * CFrame.new(0, 0, -1)
                end
            end)
            task.wait(0.05)
        end
...
rageui.ragebottab:AddToggle("Backstab", {
    Text = "always backstab",
    Default = false,
    Callback = function(val)
        config.backstab.enabled = val
        if getgenv().Hxdwp enchantedConfigLoading then return end
        if val then
            startBackstab()
        else
            stopBackstab()
...
local function createHitNotifBox(text)
    ...
    local uiScale = Hxdwp enchanted.new("UIScale")
    uiScale.Scale = 0.75
...
local function shouldNotifyPlayerHit(plr)
    ...
    local lastShot = getgenv().Hxdwp enchantedCombatLastShotAt or 0
...
local function pollHitNotifHealth()
    ...
    for _, plr in ipairs(players:GetPlayers()) do
        ...
        if cur < last - 0.01 then
            ...
            ragePerf.lastHitAtByPlayer[plr] = tick()
            if cur <= 0 then
                local tryKill = getgenv().Hxdwp enchantedTryKillSound
                if tryKill then
...
local function bindHitNotifCharacter(plr, char)
    ...
    hitNotifHumConn[plr] = hum.HealthChanged:Connect(function(newHp)
        ...
        if newHp <= 0 and localHit and localHit.plr and localHit.plr ~= player and not config.hitNotifications.enabled then
            local tryKill2 = getgenv().Hxdwp enchantedTryKillSound
            if tryKill2 then
...
if not config.profile.regionspoof then
    config.profile.regionspoof = { enabled = false, value = ".gg/getHxdwp enchanted" }
end
...
regionTab:AddInput("RegionSpoofValue", {
    Text = "region value",
    Default = ".gg/getHxdwp enchanted",
    Numeric = false,
    Finished = false,
    Placeholder = "type region...",
    Callback = function(val)
        config.profile.regionspoof.value = val or ".gg/getHxdwp enchanted"
...
v2:AddToggle("cframefly_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().Hxdwp enchantedConfigLoading then
            return
        end
        if not state then
            cleanupfly()
...
RS.Heartbeat:Connect(function(dt)
    ...
    if not flybp then
        flybp = Hxdwp enchanted.new("BodyPosition")
    ...
    if not flybg then
        flybg = Hxdwp enchanted.new("BodyGyro")
...
getgenv().Hxdwp enchantedCleanupMovement = function()
    cleanupfly()
    _G.keyheldcframe = false
    _G.keyheldcframefly = false
    local char = LP.Character
...
local function AnimPlayerGetAnimator()
    ...
    if not animator then
        animator = Hxdwp enchanted.new("Animator")
        animator.Parent = hum
    end
    return animator
end
...
local function AnimPlayerPlay()
    ...
    local anim = Hxdwp enchanted.new("Animation")
...
local fighter_controller = Hxdwp enchantedSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local local_fighter = fighter_controller.LocalFighter
local underground_enabled = false
local camera_controller = Hxdwp enchantedSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
...
local function getFloorBelowPosition(pos)
    ...
    if local_fighter and local_fighter.Entity and local_fighter.Entity.RootPart then
        raycastParams.FilterDescendantsHxdwp enchanteds = {local_fighter.Entity.RootPart.Parent}
    end
...
local function updantiaim(deltatime)
    ...
    if getgenv().Hxdwp enchantedConfigLoading then return end
    if getgenv().Hxdwp enchantedConfigLoading == nil then return end
...
getgenv().Hxdwp enchantedFlushMovementState = function()
    flushAntiAimMovementState()
    if getgenv().Hxdwp enchantedCleanupMovement then
        pcall(getgenv().Hxdwp enchantedCleanupMovement)
    else
        _G.keyheldcframe = false
        _G.keyheldcframefly = false
    end
...
getgenv().Hxdwp enchantedSyncAfterConfigLoad = function()
    if getgenv().Hxdwp enchantedFlushMovementState then
        pcall(getgenv().Hxdwp enchantedFlushMovementState)
    end
    ...
    if Toggles and Toggles.AntiAimUnderground then
        ...
        getgenv().Hxdwp enchantedUndergroundEnabled = underground_enabled
        if not underground_enabled then
            underground_oldpos = nil
        end
    else
        underground_enabled = false
        getgenv().Hxdwp enchantedUndergroundEnabled = false
        underground_oldpos = nil
    end
...
antiaimbox:AddToggle("AntiAimUnderground", {
    ...
    Callback = function(value)
        underground_enabled = value
            getgenv().Hxdwp enchantedUndergroundEnabled = value
            if getgenv().Hxdwp enchantedConfigLoading then return end
            if not value then
            underground_oldpos = nil
        end
...
getgenv().Hxdwp enchantedSetUnderground = function(val)
    underground_enabled = val
    getgenv().Hxdwp enchantedUndergroundEnabled = val
    if not val then
        underground_oldpos = nil
    end
end
...
hitNotifTab:AddToggle('HitNotifUi', {
    Text = 'ui notif',
...
local motionBlur = Hxdwp enchanted.new("BlurEffect", camera)
...
workspace.Changed:Connect(function(property)
    if property == "CurrentCamera" then
        ...
        if motionBlur and motionBlur.Parent then
            ...
            motionBlur      = Hxdwp enchanted.new("BlurEffect", camera)
            motionBlur.Name = 'QOL_MotionBlur'
            motionBlur.Size = 0
        end
...
function applyultramodaurashadersminecraft()
    ...
    if not atmosphere then
        atmosphere = Hxdwp enchanted.new("Atmosphere")
    ...
    local sunRays = Hxdwp enchanted.new("SunRaysEffect")
    ...
    local bloom = Hxdwp enchanted.new("BloomEffect")
    ...
    local cc = Hxdwp enchanted.new("ColorCorrectionEffect")
    ...
    local dof = Hxdwp enchanted.new("DepthOfFieldEffect")
    dof.Name = 'QOL_DOF'
...
local getAspectStretch = getgenv().Hxdwp enchantedGetAspectStretch
local worldToEspScreen = getgenv().Hxdwp enchantedWorldToScreenEsp or getgenv().Hxdwp enchantedWorldToScreen
local espScreenAnchor = getgenv().Hxdwp enchantedScreenAnchor
...
local espgui = Hxdwp enchanted.new("ScreenGui")
espgui.Name = "ESP"
...
local function ensureTextGradient(label, color1, color2)
    ...
    if not grad then
        grad = Hxdwp enchanted.new("UIGradient")
...
local function CreateCham(char)
    local highlight = Hxdwp enchanted.new("Highlight")
...
local function CreateBox(player)
    ...
    box.filled = Hxdwp enchanted.new("Frame")
    ...
    box.filled_gradient = Hxdwp enchanted.new("UIGradient")
    ...
    box.glow = Hxdwp enchanted.new("ImageLabel")
    ...
    box.glow_gradient = Hxdwp enchanted.new("UIGradient")
    ...
    local glowpadding = Hxdwp enchanted.new("UIPadding")
    ...
    local studsgui = Hxdwp enchanted.new("ScreenGui", game.CoreGui)
    local namegui = Hxdwp enchanted.new("ScreenGui", game.CoreGui)
    local toolgui = Hxdwp enchanted.new("ScreenGui", game.CoreGui)
...
local ESP_RENDER_BIND = "Hxdwp enchantedESPUpdate"
...
local function trackUtilDrawingText(obj)
    if getgenv().Hxdwp enchantedTrackDrawingText then
        return getgenv().Hxdwp enchantedTrackDrawingText(obj)
    end
    return obj
end
...
local function makedrawtext(text, color)
    ...
    t.Center = true
    t.Font = getgenv().Hxdwp enchantedHudDrawFont or 2
    return t
end

local function makedrawimage(imageId)
    local img = Hxdwp enchanted.new("ImageLabel")
...
local function removeespobjsfor(model)
    if utilespobjects[model] then
        ...
        if utilespobjects[model].image then
            pcall(function()
                if typeof(utilespobjects[model].image) == "Hxdwp enchanted" and utilespobjects[model].image:IsA("GuiObject") then
            ...
            if onScreen and utilconfig.enable and isselected(realname) then
                ...
                if distdraw then
                    if utilconfig.distance then
                        ...
                        distdraw.Text = getgenv().Hxdwp enchantedL("studs_short", dist)
                        if showImg then
...
local function trackBloomEffect(bloom)
    if not bloom or not bloom:IsA("BloomEffect") or bloom.Name == "Hxdwp enchantedExtraBloom" then
        return
    end
...
local function applyBloomModifier()
    ...
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("BloomEffect") and child.Name ~= "Hxdwp enchantedExtraBloom" then
            child.Enabled = false
        end
    end

    if bloomCfg.Enabled then
        local extraBloom = Lighting:FindFirstChild("Hxdwp enchantedExtraBloom")
        if not extraBloom then
            extraBloom = Hxdwp enchanted.new("BloomEffect")
            extraBloom.Name = "Hxdwp enchantedExtraBloom"
        ...
        local legacyExtra = Lighting:FindFirstChild("Hxdwp enchantedExtraBloom")
        if legacyExtra then
            legacyExtra:Destroy()
        end
        for _, child in ipairs(Lighting:GetChildren()) do
...
local function ensureExtraEffects()
    ...
    if ccCfg and ccCfg.Enabled then
        if not extraColorCorrection or not extraColorCorrection.Parent then
            extraColorCorrection = Hxdwp enchanted.new("ColorCorrectionEffect")
            extraColorCorrection.Name = "Hxdwp enchantedExtraColorCorrection"
...
local function stoplightupdater()
    ...
    if not shadersActive() then
        ...
        if origatmo then
            local origatm = Hxdwp enchanted.new("Atmosphere")
...
local WATERMARK_TEXT = "Hxdwp enchanted"

local textChars = {}
for i = 1, #WATERMARK_TEXT do
    local char = getgenv().Hxdwp enchantedTrackDrawingText(Drawing.new("Text"))
    char.Size = 13
    char.Font = 2
    char.Outline = true
...
local ammoLabel = getgenv().Hxdwp enchantedTrackDrawingText(Drawing.new("Text"))
ammoLabel.Size = 13
ammoLabel.Font = 2
...
local function applycustomatmo()
    ...
    local atm = Hxdwp enchanted.new("Atmosphere")
...
local function playambientsound()
    ...
    curambientsound = Hxdwp enchanted.new("Sound")
...
getgenv().Hxdwp enchantedPlayKillSound = playKillSound

getgenv().Hxdwp enchantedTryKillSound = function(plr, lastHp, newHp)
    if not ksCfg.Enabled then
        return
    end
    if not plr or plr == ksLocalPlayer then
...
local function ormakeskybox()
    if not customSky then
        customSky = Hxdwp enchanted.new("Sky")
...
local ASPECT_RATIO_BIND = "Hxdwp enchantedAspectRatio"
...
getgenv().Hxdwp enchantedStopSkyboxUpdater = stopSkyboxUpdater
...
local function build(key)
    ...
    local part = Hxdwp enchanted.new("Part")
    ...
    local p = Hxdwp enchanted.new("ParticleEmitter")
    applyparticle(p, cfg)
    p.Parent = part
...
game:GetService("ScriptContext").DescendantRemoving:Connect(function(descendant)
    if descendant == script then
        local stopSkyboxUpdater = getgenv().Hxdwp enchantedStopSkyboxUpdater
...
local WatermarkConnection = game:GetService('RunService').Heartbeat:Connect(function()
    ...
    if (tick() - FrameTimer) >= 1 then
        ...
        FrameTimer = tick()
        FrameCounter = 0
        Library:SetWatermark(('Hxdwp enchanted | %s fps'):format(
            math.floor(FPS)
        ))
    end
end)

Library:SetWatermark('Hxdwp enchanted')
...
Library:OnUnload(function()
    ...
    getgenv().Hxdwp enchantedKeybindList = nil
    Library.Unloaded = true
end)

Library.KeybindFrame.Visible = false

getgenv().Hxdwp enchantedKeybindList = getgenv().Hxdwp enchantedKeybindList or {}
local kbList = getgenv().Hxdwp enchantedKeybindList
local KB_ROW_H = 18
local KB_PAD = 8
local KB_TITLE_H = 28
...
local function kbTextWidth(text, textSize)
    ...
    local ok, sz = pcall(function()
        local fontFace = getgenv().Hxdwp enchantedGetUiFont and getgenv().Hxdwp enchantedGetUiFont()
...
local function buildKbListUi()
    ...
    local gui = Hxdwp enchanted.new("ScreenGui")
    gui.Name = "Hxdwp enchanted_keybind_list"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    ...
    local outer = Hxdwp enchanted.new("Frame")
    outer.Name = "KbOuter"
    ...
    local inner = Hxdwp enchanted.new("Frame")
    inner.Name = "KbInner"
    ...
    local titleBar = Hxdwp enchanted.new("Frame")
    titleBar.BackgroundTransparency = 1
    titleBar.Size = UDim2.new(1, 0, 0, KB_TITLE_H)
    titleBar.Parent = inner

    local title = Hxdwp enchanted.new("TextLabel")
    title.Name = "Title"
    ...
    getgenv().Hxdwp enchantedApplyUiFont(title, 14)

    local divider = Hxdwp enchanted.new("Frame")
    ...
    local scroll = Hxdwp enchanted.new("ScrollingFrame")
    scroll.Name = "List"
    ...
    local layout = Hxdwp enchanted.new("UIListLayout")
    ...
    local scrollPad = Hxdwp enchanted.new("UIPadding")
    ...
    for i = 1, KB_MAX_ROWS do
        local rowFrame = Hxdwp enchanted.new("Frame")
        ...
        local lbl = Hxdwp enchanted.new("TextLabel")
        lbl.BackgroundTransparency = 1
        lbl.Size = UDim2.new(1, 0, 1, 0)
        ...
        lbl.Parent = rowFrame
        getgenv().Hxdwp enchantedApplyUiFont(lbl, 12)
...
applyKbListTheme = function()
    ...
    if kbList.title then
        ...
        getgenv().Hxdwp enchantedApplyUiFont(kbList.title, 14)
    end
    ...
    for _, row in ipairs(kbList.rowPool) do
        if row.lbl then
            ...
            getgenv().Hxdwp enchantedApplyUiFont(row.lbl, 12)
        end
        if row.frame then
...
getgenv().Hxdwp enchantedApplyKbListTheme = function()
    kbThemeDirty = true
    applyKbListTheme()
end
getgenv().Hxdwp enchantedRefreshKbList = function(force)
...
do
    local prevSync = getgenv().Hxdwp enchantedSyncAfterConfigLoad
    getgenv().Hxdwp enchantedSyncAfterConfigLoad = function()
...
local function syncMenuOverlayUi()
    ...
    if open and getgenv().Hxdwp enchantedCosmeticUIState and getgenv().Hxdwp enchantedCosmeticUIState.applyLiveViewModelUpdate then
        task.defer(function()
            pcall(getgenv().Hxdwp enchantedCosmeticUIState.applyLiveViewModelUpdate)
        end)
    end
    if open and getgenv().Hxdwp enchantedApplyCosmeticUiTheme then
        pcall(getgenv().Hxdwp enchantedApplyCosmeticUiTheme)
    end
end
...
getgenv().Hxdwp enchantedCenterWindow = function()
    if not Window or not Window.Holder then return end
    pcall(function()
        local cam = workspace.CurrentCamera
        if cam then
...
task.defer(function()
    ...
    if cam then
        ...
        if vs.X < 1280 or vs.Y < 720 then
            ...
            if getgenv().Hxdwp enchantedCenterWindow then getgenv().Hxdwp enchantedCenterWindow() end
        end
    end
end)
...
MenuGroup:AddDropdown('Hxdwp enchantedLanguage', {
    Text = 'language',
    Values = { 'english', 'korean', 'spanish' },
    Default = getgenv().Hxdwp enchantedLanguage or 'english',
    Callback = function(val)
        getgenv().Hxdwp enchantedLanguage = val
        if getgenv().Hxdwp enchantedReloadLanguageFont then
            getgenv().Hxdwp enchantedReloadLanguageFont(val)
        end
        if getgenv().Hxdwp enchantedApplyUIFont then
            getgenv().Hxdwp enchantedApplyUIFont()
        end
        if getgenv().Hxdwp enchantedRefreshLocaleUI then
            pcall(getgenv().Hxdwp enchantedRefreshLocaleUI)
        end
    end,
})
...
ServerGroup:AddToggle('ACDebug', {
    ...
    Callback = function(v)
        getgenv().Hxdwp enchantedACDebug = v
    end,
})
...
ServerGroup:AddButton('Rejoin Server', function()
    if game.JobId ~= "" then
        ...
        TeleportService:TeleportToPlaceHxdwp enchanted(game.PlaceId, game.JobId, LocalPlayer)
    else
...
ServerGroup:AddButton('Server Hop (Random)', function()
    ...
    TeleportService:TeleportToPlaceHxdwp enchanted(game.PlaceId, s.id, LocalPlayer)
end)
...
(function()
    ...
    local function isRigHxdwp enchanted(obj)
        return typeof(obj) == "Hxdwp enchanted" and obj:IsA("Model")
    end
    ...
    local function getServerRigs()
        ...
        if liveFolder and typeof(liveFolder) == "Hxdwp enchanted" then
            local added = {}
            local function tryAdd(rig)
                if isRigHxdwp enchanted(rig) and rig.Parent and not added[rig] then
                    added[rig] = true
                    table.insert(serverRigCache, rig)
                end
            end
            if isRigHxdwp enchanted(liveFolder) then
                tryAdd(liveFolder)
            end
            pcall(function()
                for _, child in liveFolder:GetChildren() do
                    if isRigHxdwp enchanted(child) and (child:FindFirstChild("HumanoidRootPart") or child:FindFirstChild("RootPart")) then
                        tryAdd(child)
                    end
                end
            end)
            if #serverRigCache == 0 and isRigHxdwp enchanted(liveFolder) then
                tryAdd(liveFolder)
            end
        end

        local entity = getFighterEntity()
        if isRigHxdwp enchanted(entity) then
            local dup = false
    ...
    local function deepDiveGetAnimObject(bundleId)
        ...
        for _, topObject in pairs(objects) do
            if typeof(topObject) == "Hxdwp enchanted" and topObject:IsA("Animation") then
                animObj = topObject
            elseif typeof(topObject) == "Hxdwp enchanted" then
        ...
        local fallback = Hxdwp enchanted.new("Animation")
...
do
    local oldBlur = Lighting:FindFirstChild("Hxdwp enchantedTextureBlur")
    if oldBlur then
        oldBlur:Destroy()
    end
end
...
local smoothTexSaStore = Hxdwp enchanted.new("Folder")
...
local function Hxdwp enchantedHasTargetTexture(inst)
...
local function recordTargetTexture(inst)
    if not inst or targetTextureOrig[inst] or not Hxdwp enchantedHasTargetTexture(inst) then return end
...
local function degradeTextureHxdwp enchanted(inst)
    if not inst or not Hxdwp enchantedHasTargetTexture(inst) then return end
...
local function processTargetTextureHxdwp enchanted(inst)
    if not inst then return end
    if Hxdwp enchantedHasTargetTexture(inst) then degradeTextureHxdwp enchanted(inst) end
    for _, child in ipairs(inst:GetDescendants()) do
        if Hxdwp enchantedHasTargetTexture(child) then degradeTextureHxdwp enchanted(child) end
    end
end
...
local function startTargetTextureFilter()
    ...
    targetTextureConn = game.DescendantAdded:Connect(function(inst)
        task.wait(0.1)
        processTargetTextureHxdwp enchanted(inst)
    end)
end
...
getgenv().Hxdwp enchantedUpdSmoothTextures = updSmoothTextures
getgenv().Hxdwp enchantedUpdDarkTextures = updDarkTextures
getgenv().Hxdwp enchantedUpdTransparentTextures = updTransparentTextures
...
texturesBox:AddToggle('smooth_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().Hxdwp enchantedUpdSmoothTextures
        if fn then fn() end
    end
})
...
texturesBox:AddToggle('dark_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().Hxdwp enchantedUpdDarkTextures
        if fn then fn() end
    end
})
...
local transparentToggle = texturesBox:AddToggle('transparent_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().Hxdwp enchantedUpdTransparentTextures
        if fn then fn() end
    end
})
...
function ThemeManager:ThemeUpdate()
    ...
    if getgenv().Hxdwp enchantedApplyCosmeticUiTheme then
        pcall(getgenv().Hxdwp enchantedApplyCosmeticUiTheme)
    end
    if getgenv().Hxdwp enchantedKeybindList and getgenv().Hxdwp enchantedKeybindList.inner then
        if getgenv().Hxdwp enchantedApplyKbListTheme then
            pcall(getgenv().Hxdwp enchantedApplyKbListTheme)
        end
    end
end
...
local _origSave = SaveManager.Save
function SaveManager:Save(name, ...)
    pcall(function()
        if isfolder and makefolder then
            if not isfolder('Hxdwp enchanted') then makefolder('Hxdwp enchanted') end
            if not isfolder('Hxdwp enchanted/rivals') then makefolder('Hxdwp enchanted/rivals') end
            if not isfolder('Hxdwp enchanted/rivals/settings') then makefolder('Hxdwp enchanted/rivals/settings') end
        end
    end)
    local saveOk, success, err = pcall(_origSave, self, name, ...)
    if not saveOk then
        Library:Notify('Save failed: ' .. tostring(success), 5)
        return false, tostring(success)
    end
    return success, err
end

local _origRefresh = SaveManager.RefreshConfigList
function SaveManager:RefreshConfigList(...)
    local ok, result = pcall(_origRefresh, self, ...)
    if not ok then
        Library:Notify('Refresh failed: ' .. tostring(result), 5)
        return {}
    end
    return result
end
...
ThemeManager:SetFolder('Hxdwp enchanted')
SaveManager:SetFolder('Hxdwp enchanted/rivals')
...
do
    local ConfigListGroup = Tabs.Configs:AddLeftGroupbox('config files')
    local configBtnRefs = {}
    ...
    local ConfigManageGroup = Tabs.Configs:AddRightGroupbox('manage')
...
getgenv().Hxdwp enchantedConfigLoading = true
applyHxdwp enchantedAccentTheme()

task.defer(function()
    task.wait(0.35)
    pcall(applyHxdwp enchantedAccentTheme)
    pcall(function()
    ...
    task.wait(1.25)
    pcall(applyHxdwp enchantedAccentTheme)
    getgenv().Hxdwp enchantedConfigLoading = false
end)
end)()