...
do
    ...
    function Library:MakeDraggable(Instance, Cutoff)
        ...
        Instance.InputBegan:Connect(function(Input)
            ...
            local dX = mPos.X - Instance.AbsolutePosition.X
            local dY = mPos.Y - Instance.AbsolutePosition.Y
            if dY > (Cutoff or 40) then return end
            while UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                local mp = UIS:GetMouseLocation()
                Instance.Position = UDim2.fromOffset(mp.X - dX, mp.Y - dY)
...
function SaveManager:Save(name, ...)
    pcall(function()
        if isfolder and makefolder then
            if not isfolder('instance') then makefolder('instance') end
            if not isfolder('instance/rivals') then makefolder('instance/rivals') end
            if not isfolder('instance/rivals/settings') then makefolder('instance/rivals/settings') end
        end
    end)
